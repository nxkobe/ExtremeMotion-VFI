...
class FlowRefine(nn.Module):
    ...
    def forward(self, x, flow_s, flow):
        if self.scale != 1:
            x = F.interpolate(x, scale_factor=1. / self.scale, mode="bilinear", align_corners=False).contiguous()
        if flow is not None:
            flow = F.interpolate(flow, scale_factor=1. / self.scale, mode="bilinear",
                                 align_corners=False).contiguous() * 1. / self.scale
            x = torch.cat((x, flow), 1).contiguous()
